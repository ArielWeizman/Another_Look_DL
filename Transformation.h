...
public:
	...
	*/
	virtual void operator()(uint8_t * state) const = 0;
	/**
...
public:
	...
	*                 ceil(getWidth()/8.0) bytes.
	*/
	virtual void inverse(uint8_t * state) const = 0;
};
...
public:
	...
	virtual void operator()(uint8_t * state) const { (void)state; }
	virtual string getDescription() const { return "Identity"; }
	virtual void inverse(uint8_t * state) const { (void)state; }
};
